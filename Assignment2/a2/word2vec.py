...
def naiveSoftmaxLossAndGradient(
    ...
    # to integer overflow.``
    predictedDist = softmax(np.matmul(outsideVectors, centerWordVec))  # (v,)
    loss = -np.log(predictedDist[outsideWordIdx])
    gradCenterVec = np.matmul(outsideVectors.T,predictedDist) - outsideVectors[outsideWordIdx] # (l,)
    gradOutsideVecs = np.matmul(predictedDist.reshape((predictedDist.shape[0], 1)), centerWordVec.T.reshape((1, centerWordVec.shape[0])))  # (v,l)
    gradOutsideVecs[outsideWordIdx] -= centerWordVec  # (l,)
    # END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    M = np.matmul(outsideVectors[indices], centerWordVec) # U_0 * v_c (w x 1)
    negSampleVectors = sigmoid(M) # O(U_0 * v_c) (w x 1)
    loss = - np.log(negSampleVectors[0]) - np.sum(np.log(sigmoid(-M[1:]))) # -log(O(u_o * v_c)) - sum(log(O(-u_s * v_c)))
    gradCenterVec = outsideVectors[outsideWordIdx] * (negSampleVectors[0] - 1) + np.matmul(np.transpose(negSampleVectors[1:]), outsideVectors[indices[1:]])
    grad = np.matmul(negSampleVectors.reshape((negSampleVectors.shape[0], 1)), centerWordVec.T.reshape((1, centerWordVec.shape[0])))
    grad[0] -= centerWordVec
    _sorted = sorted(indices)
    idx = 0
    gradOutsideVecs = np.zeros_like(outsideVectors)
    while (idx < len(_sorted)):
        count = _sorted.count(_sorted[idx])
        gradOutsideVecs[_sorted[idx]] = grad[indices.index(_sorted[idx])] * count
        idx += count
    # END YOUR CODE
    return loss, gradCenterVec, gradOutsideVecs
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerWordVec = centerWordVectors[c]
    for word in outsideWords:
        output = word2vecLossAndGradient(centerWordVec, word2Ind[word], outsideVectors, dataset)
        loss += output[0]
        gradCenterVecs[c] += output[1]
        gradOutsideVectors += output[2]
    # END YOUR CODE
